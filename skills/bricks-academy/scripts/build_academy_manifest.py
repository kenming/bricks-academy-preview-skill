...
def main() -> None:
    ...
    write_csv(OUT_DIR / "academy_manifest.csv", rows)
    (OUT_DIR / "academy_manifest.json").write_text(
        json.dumps(rows, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    print(f"wrote {len(rows)} Academy docs to {OUT_DIR / 'academy_manifest.csv'}")
