...
MANIFEST_PATH = SKILL_DIR / "index" / "preview_manifest.csv"
CACHE_PATH = SKILL_DIR / "index" / "preview_remote_etags.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Lightweight Bricks Academy preview update check using remote .md ETags."
    )
    parser.add_argument(
        "--update-cache",
        action="store_true",
    ...
    if args.json:
        ...
        if new_remote_paths:
            print("\nNew paths on official preview site:")
            for path in new_remote_paths[:30]:
                print(f"- {BASE_URL}{path}")
        if missing_remote_paths:
            print("\nManifest paths not found on official preview homepage:")
